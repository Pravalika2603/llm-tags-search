...
def search(req: SearchRequest, db: Session = Depends(get_session)):
    # ensure list[float], not numpy array
    raw_qvec = embed_query(req.query)
    qvec = [float(x) for x in raw_qvec]
    ...
    kw AS (
      SELECT c.id AS chunk_id, c.doc_id, c.chunk_idx, c.text, c.heading, c.page,
             ts_rank_cd(c.tsv, websearch_to_tsquery('english', :query)) AS kw_score
      FROM chunks c
      JOIN documents d ON d.id = c.doc_id
      WHERE {where_sql} AND c.tsv @@ websearch_to_tsquery('english', :query)
    ...
    ranked = (rerank(req.query, candidates, top_k=req.k)
              if len(candidates) > req.k
              else sorted(candidates, key=lambda x: x["score"], reverse=True)[:req.k])

    hits = [SearchHit(
        doc_id=h["doc_id"], chunk_id=h["chunk_id"], title=h["title"],
        heading=h["heading"], page=h["page"], text=h["text"][:1200],
        score=h.get("rerank_score", h["score"]), tags=h["tags"]
    ) for h in ranked]
